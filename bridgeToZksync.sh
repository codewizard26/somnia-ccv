...
# Script to bridge tokens from Sepolia to 0G Galileo Testnet
...
GALILEO_POOL_ADDRESS=$(forge create src/RebaseTokenPool.sol:RebaseTokenPool --rpc-url ${GALILEO_RPC_URL} --private-key ${GALILEO_PRIVATE_KEY}  --constructor-args ${GALILEO_REBASE_TOKEN_ADDRESS} [] ${GALILEO_RNM_PROXY_ADDRESS} ${GALILEO_ROUTER} | awk '/Deployed to:/ {print $3}')
...
# Test the pool configuration before configuring
...
VAULT_ADDRESS=$(forge script ./script/Deployer.s.sol:VaultDeployer --rpc-url ${SEPOLIA_RPC_URL} --private-key ${PRIVATE_KEY} --broadcast --sig "run(address)" ${SEPOLIA_REBASE_TOKEN_ADDRESS} | grep 'vault: contract Vault' | awk '{print $NF}')
...
if [ "$SEPOLIA_CHAIN_SUPPORTED" != "true" ]; then
...
# Test the pool configuration after configuring
...
cast send ${VAULT_ADDRESS} --value ${AMOUNT} --rpc-url ${SEPOLIA_RPC_URL} --private-key ${PRIVATE_KEY} "deposit()"
...
if [ "$GALILEO_CHAIN_SUPPORTED" != "true" ]; then
...
# Bridge the funds using the script to Galileo 
echo "Bridging the funds using the script to Galileo..."
SENDER_ADDRESS=$(cast wallet address --private-key ${PRIVATE_KEY})
SEPOLIA_BALANCE_BEFORE=$(cast balance ${SENDER_ADDRESS} --erc20 ${SEPOLIA_REBASE_TOKEN_ADDRESS} --rpc-url ${SEPOLIA_RPC_URL})
echo "Sepolia balance before bridging: $SEPOLIA_BALANCE_BEFORE"
forge script ./script/BridgeTokens.s.sol:BridgeTokensScript --rpc-url ${SEPOLIA_RPC_URL} --private-key ${PRIVATE_KEY} --broadcast --sig "run(address,uint64,address,uint256,address,address)" ${SENDER_ADDRESS} ${GALILEO_CHAIN_SELECTOR} ${SEPOLIA_REBASE_TOKEN_ADDRESS} ${AMOUNT} ${SEPOLIA_LINK_ADDRESS} ${SEPOLIA_ROUTER}
echo "Funds bridged to Galileo"
SEPOLIA_BALANCE_AFTER=$(cast balance ${SENDER_ADDRESS} --erc20 ${SEPOLIA_REBASE_TOKEN_ADDRESS} --rpc-url ${SEPOLIA_RPC_URL})
